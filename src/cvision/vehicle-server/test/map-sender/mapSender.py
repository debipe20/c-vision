"""
Usage:  
python3 mapsender.py 
python3 mapsender.py once
"""
...
def main(loop=True):
    ...
    port = config["PortNumber"]["MapSender"]
    ...
    send_period = 1.0  # seconds
    next_time = time.perf_counter()

    try:
         # choose iterator based on loop flag
        ...
        for fname in file_iter:
            ...
            try:
                with open(fname, "r", encoding="utf-8") as f:
                    data = f.read()
                map_sender_socket.sendto(data.encode("utf-8"), client_info)
